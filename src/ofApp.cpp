...
void ofApp::onGifSaved(string & fileName){
    ...
    string tempPath= ofToDataPath(fileName);
    ofFile temp(tempPath);
    if (temp.exists()){
...
//    ofxGuiSetDefaultHeight(100);
    ...
    experimentImg.load("experiment.png");
    recordingImg.load("recording.png");
	processingImg.load("exporting.png");
    closeImg.load("close.png");
...
void ofApp::draw(){
	...
	if (!bRecordingMode){
		...
		ofSetColor(0);
		ofFill();
		ofDrawRectangle(0,0,wholeScreen->getWidth(), wholeScreen->getHeight());
	}
	ofSetColor(255);
    ...
    ofDrawLine(0,0,normAccel.x*screenWidth/2,0);
    //ofSetColor(255,0,255, 0);
    ofDrawLine(0,0,0,-normAccel.y*screenHeight/2);
    ...
    if (bRecordingMode == true){
    	...
    	ofDrawRectangle(tempRect.x, tempRect.y,tempRect.width * recordPct, tempRect.height);
    ...
    if(bRecordingMode == true){
        ...
        ofRectangle tempRect(screenWidth/2 - recordingImg.getWidth()/2, screenHeight-100 - recordingImg.getHeight()/2, recordingImg.getWidth(), recordingImg.getHeight());
